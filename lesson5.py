...
# видите что произошло? список в списке!
# внутренний список стал одним из элементов списка
# и если мы хотим вывести его, то можем обратиться по его индексу
ocenki = [4, 5, 3, 4, 5, [3, 3, 3]]
print(ocenki[6]) #[3, 3, 3]

# а если мы хотим вывести 3 из этого внутреннего списка, то нужно указать его индекс вот таким образом:
ocenki = [4, 5, 3, 4, 5, [3, 3, 3]]
print(ocenki[6][0]) # 3

# по такой же логике мы можем добавлять новые элементы к внутреннему списку
ocenki = [4, 5, 3, 4, 5, [3, 3, 3]]
ocenki[6].append(5)
print(ocenki) # [4, 5, 3, 4, 5, [3, 3, 3, 5]]


# а как изменить элемент списка?
# например, мы хотим исправить 3 на 5
ocenki = [4, 5, 3, 4, 5]
# образаемся к этой тройке по индексу и передаём туда новое значение:
ocenki[2] = 5
# и теперь наш список выглядит вот так:
print(ocenki) # [4, 5, 5, 4, 5]
